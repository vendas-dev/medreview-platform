...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ error: 'Payload inválido', details: parsed.error.flatten() }, { status: 400, headers: CORS })
    }
    ...
    let isSelfCO        = false
    let finalSellerType = (data.seller_type ?? 'closer') as string

    // Match: hubspot_id → nome → self checkout
    ...
    if (!matched && !rawHubspotId && !rawName) {
      isSelfCO = true; finalSellerType = 'self_checkout'
    }
    ...
    let insertData: Record<string, unknown> = {
      ...
      // ← Sempre salva o hubspot_id do closer no evento
      // Permite vincular retroativamente quando o closer se cadastrar depois
      closer_hubspot_id:   isSelfCO ? null : (rawHubspotId || null),
      is_self_checkout:    isSelfCO,
      seller_type:         finalSellerType,
      sold_by_ambassador:  finalSellerType === 'ambassador',
      occurred_at:         occurredAt,
    }

    if (data.event_type === 'sale') {
      Object.assign(insertData, { lead_name: data.lead_name, product: (data.product ?? ''), value: data.value })
    } else {
      Object.assign(insertData, { ambassador_name: data.ambassador_name, college: (data.college ?? ''), class: (data.class ?? '') })
    }
    ...
    return NextResponse.json({
      ...
      closer_hubspot_id: (matched?.hubspot_id ?? null) || (rawHubspotId || null),
      is_self_checkout:  isSelfCO,
      seller_type:       finalSellerType,
