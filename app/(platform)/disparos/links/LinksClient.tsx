...
import { dateInSaoPaulo, hourInSaoPaulo, todayInSaoPaulo, dayBoundsSaoPaulo } from '@/lib/timezone'
import {
...
export function LinksClient({ isAdmin, ownerName, ownerHubspotId }:{ isAdmin:boolean; ownerName?:string; ownerHubspotId?:string|null }) {
  ...
  const byProduct=useMemo(()=>{
    const m:Record<string,{count:number;product:string;vertical:string}>={}
    filtered.forEach(d=>{
      const p=d.product_name??'S/produto'
      const v=d.vertical??'S/vertical'
      const key=`${p}|||${v}`
      if(!m[key])m[key]={count:0,product:p,vertical:v}
      m[key].count++
    })
    return Object.values(m).sort((a,b)=>b.count-a.count).slice(0,12).map(d=>({
      name:`${d.product.length>18?d.product.slice(0,18)+'…':d.product} · ${d.vertical}`,
      count:d.count, product:d.product, vertical:d.vertical,
    }))
  },[filtered])
          ...
          {tab==='geral'&&(
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                <div style={CARD}>
                  ...
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart onClick={(p:any)=>p?.activePayload?.[0]&&setDrill({title:`Vertical: ${p.activePayload[0].name}`,rows:filtered.filter(d=>(d.vertical??'S/vertical')===p.activePayload[0].name)})}>
                      <Pie data={byVertical} dataKey="value" nameKey="name" cx="50%" cy="45%" outerRadius={62} label={({percent})=>`${((percent as number)*100).toFixed(0)}%`} labelLine={false}>
                ...
                <div style={CARD}>
                  <h3 style={TIT}>Por Condição</h3><p style={SUB}>À vista, parcelado ou Nx sem juros</p>
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={byCondition} onClick={(p:any)=>p?.activePayload?.[0]&&setDrill({title:`Condição: ${p.activePayload[0].payload.name}`,rows:filtered.filter(d=>(conditionLabel(d)??'S/condição')===p.activePayload[0].payload.name)})}>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--border)"/>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div style={CARD}>
                ...
                <ResponsiveContainer width="100%" height={Math.max(280,byProduct.length*32)}>
                  <BarChart data={byProduct} layout="vertical" margin={{left:10,right:60}} onClick={(p:any)=>p?.activePayload?.[0]&&setDrill({title:`Produto: ${p.activePayload[0].payload.name}`,rows:filtered.filter(d=>(d.product_name??'S/produto')===p.activePayload[0].payload.product&&(d.vertical??'S/vertical')===p.activePayload[0].payload.vertical)})}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" horizontal={false}/>
                    <XAxis type="number" tick={{fontSize:10,fill:'var(--muted-foreground)'}}/>
                    <YAxis type="category" dataKey="name" tick={{fontSize:10,fill:'var(--foreground)'}} width={210}/>
                    <Tooltip contentStyle={TOOLTIP_STYLE}/>
                    <Bar dataKey="count" radius={[0,6,6,0]} name="Links">
              ...
              <RankingList data={byProduct} label="Ranking de Produtos" onClick={n=>{
                const info=byProduct.find(p=>p.name===n)
                setDrill({title:`Produto: ${n}`,rows:info?filtered.filter(d=>(d.product_name??'S/produto')===info.product&&(d.vertical??'S/vertical')===info.vertical):[]})
              }}/>
            </div>
          )}
