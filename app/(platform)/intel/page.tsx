...
// Mapeamento de slug interno → label de exibição
...
function parseInsight(raw: string | null): any | null {
  if (!raw) return null
  try { return JSON.parse(raw) } catch { return { resumo_time: raw } }
}
...
export default async function IntelPage() {
  ...
  const bizPassed = bizDays(new Date(y, m - 1, 1), now)
  const bizLeft   = Math.max(bizTotal - bizPassed, 0)
  ...
  if (isAdmin) {
    const [
      ...
      admin.from('telao_events').select('closer_id, closer_name, closer_hubspot_id, value, vertical, occurred_at').eq('event_type','sale').gte('occurred_at', mStart).lte('occurred_at', mEnd),
    ...
    const closerStats = (closers ?? []).map((c: any) => {
      const mySales = matchSales(allSales ?? [], c)
      ...
      const sorted     = [...mySales].sort((a: any, b: any) => (b.occurred_at ?? '').localeCompare(a.occurred_at ?? ''))
      const daysSince  = sorted[0]?.occurred_at ? Math.floor((Date.now() - new Date(sorted[0].occurred_at).getTime()) / 86400000) : 999
      const byVert: Record<string, number> = {}
      mySales.forEach((e: any) => { if (e.vertical) { const k = vLabel(e.vertical); byVert[k] = (byVert[k] ?? 0) + (Number(e.value) || 0) } })

      return { id:c.id, name:c.name, team:c.team, revenue_month:revMonth, sales_month:mySales.length, goal_sales:goalSales, pct_goal:pctGoal, revenue_projected:projected, leads_month:leadsTotal, leads_open:leadsOpen, conversion_rate:convRate, days_since_last_sale:daysSince, revenue_by_vertical:byVert, biz_total:bizTotal, biz_passed:bizPassed, biz_left:bizLeft }
    })

    return <IntelView isAdmin={true} profile={profile as any}
      closerStats={closerStats} insightData={parseInsight(insightRow?.content ?? null)}
      insightDate={today} snapshot={null}/>
  }
  ...
  const [
    ...
    hubId ? admin.from('telao_events').select('id, value, vertical, occurred_at').eq('event_type','sale').eq('closer_hubspot_id', hubId).gte('occurred_at', mStart).lte('occurred_at', mEnd)
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const rev = salesM.filter((e: any) => { const t = new Date(e.occurred_at).getTime(); return t >= d.getTime() && t <= de.getTime() }).reduce((s: number, e: any) => s + (Number(e.value) || 0), 0)
    last7.push({ label: ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][d.getDay()], rev })
  }
