  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 40, padding: '0 18px', borderRadius: 10, background: 'var(--foreground)', color: 'var(--card)', fontSize: 14, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'opacity 0.15s' }}>
        {mode === 'create' ? <><Plus size={15} /> Nova etapa</> : 'Editar'}
      </button>
      ...
      {open && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
            ...
            <form action={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div>
                ...
                <input name="title" required defaultValue={step?.title} placeholder="Ex: Conhecer a empresa" style={inputStyle} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select name="team" defaultValue={step?.team ?? 'ambos'} style={{ ...inputStyle, appearance: 'none', paddingRight: 36, cursor: 'pointer' }}>
