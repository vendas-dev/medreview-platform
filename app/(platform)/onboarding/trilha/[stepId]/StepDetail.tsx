...
function MaterialCardInline({ mat, isAdmin, onDeleted, onUpdated, onCheck, allSteps = [] }: any) {
  ...
  return (
    <>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 22, overflow: 'hidden', transition: 'all 0.18s', boxShadow: 'var(--shadow-xs)', display: 'flex', flexDirection: 'column', opacity: checked ? 0.65 : 1 }}
        onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = tc.color + '50'; el.style.boxShadow = `0 6px 20px ${tc.color}14, var(--shadow-sm)`; el.style.transform = 'translateY(-2px)' }}
        ...
        {thumb && mat.type === 'video' ? (
          <div onClick={() => setPlaying(true)} style={{ position: 'relative', height: 152, cursor: 'pointer', background: '#000', overflow: 'hidden', flexShrink: 0 }}>
            <img src={thumb} alt={mat.title} style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.8 }} />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,transparent 50%,rgba(0,0,0,.45) 100%)' }} />
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(255,255,255,0.94)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(0,0,0,.3)' }}>
                <Play size={20} style={{ color: '#111', marginLeft: 3 }} />
              </div>
            </div>
            {checked && <div style={{ position: 'absolute', top: 10, right: 10, width: 26, height: 26, borderRadius: '50%', background: '#22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px rgba(0,0,0,.3)' }}><Check size={14} style={{ color: '#fff' }} /></div>}
          </div>
        ) : (
        ...
        <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', flex: 1 }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8, minHeight: 20 }}>
            <span style={{ fontSize: 9.5, fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: tc.color + '18', color: tc.color }}>{tc.label}</span>
            {mat.day_number && <span style={{ fontSize: 9.5, fontWeight: 800, padding: '3px 9px', borderRadius: 999, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff' }}>Dia {mat.day_number}</span>}
            {checked && !thumb && <span style={{ fontSize: 9.5, fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: 'rgba(34,197,94,0.1)', color: '#16a34a', border: '1px solid rgba(34,197,94,0.2)' }}>✓ Concluído</span>}
          </div>
          <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--foreground)', margin: '0 0 5px', lineHeight: 1.4 }}>{mat.title}</p>
          <p style={{ fontSize: 12, color: 'var(--muted-foreground)', margin: 0, lineHeight: 1.55, flex: 1, minHeight: 0 }}>{mat.description ?? ''}</p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
            {!(thumb && mat.type === 'video') && (
              <button onClick={() => embedUrl ? setPlaying(true) : window.open(mat.url, '_blank')} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, height: 34, padding: '0 16px', borderRadius: 999, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'inherit', boxShadow: '0 3px 10px rgba(79,70,229,0.3)' }}>
                {mat.type === 'video' ? <><Play size={11} /> Assistir</> : <><ExternalLink size={11} /> Abrir</>}
              </button>
            )}
            {!isAdmin && (<div style={{ marginLeft: 'auto' }}><CheckButton checked={checked} onClick={handleCheck} label="Marcar concluído" /></div>)}
            {isAdmin && (
              <div style={{ display: 'flex', gap: 4, marginLeft: 'auto', opacity: .55, transition: 'opacity .15s' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = '1'}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = '.55'}>
                <button onClick={() => setEditing(true)} style={{ width: 26, height: 26, borderRadius: 999, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(99,102,241,0.1)'; e.currentTarget.style.color = '#6366f1' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)' }}><Pencil size={10} /></button>
                <button onClick={() => setDeleting(true)} style={{ width: 26, height: 26, borderRadius: 999, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(239,68,68,0.08)'; e.currentTarget.style.color = '#ef4444' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)' }}><Trash2 size={10} /></button>
              </div>
            )}
          </div>
        </div>
...
function FaqCard({ faq, isAdmin, expanded, onToggle, onDeleted, onUpdated }: any) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 18, overflow: 'hidden', boxShadow: 'var(--shadow-xs)', transition: 'border-color 0.15s' }} onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(99,102,241,0.2)'} onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={onToggle} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px 18px', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', gap: 10 }}>
          <span style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--foreground)' }}>{faq.question}</span>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', gap: 4, paddingRight: 12, flexShrink: 0, opacity: .55 }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = '1'}
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = '.55'}>
            <button onClick={() => setEditing(true)} style={{ width: 26, height: 26, borderRadius: 999, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(99,102,241,0.1)'; e.currentTarget.style.color = '#6366f1' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)' }}><Pencil size={10} /></button>
            <button onClick={() => setDeleting(true)} style={{ width: 26, height: 26, borderRadius: 999, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(239,68,68,0.08)'; e.currentTarget.style.color = '#ef4444' }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)' }}><Trash2 size={10} /></button>
      ...
      {expanded && <div style={{ padding: '10px 18px 16px', fontSize: 13.5, color: 'var(--muted-foreground)', lineHeight: 1.7, borderTop: '1px solid var(--border)' }}>{faq.answer}</div>}
    </div>
  )
}
...
export function StepDetail({
  ...
  const hasNoContent    = initMaterials.length === 0 && initQuestions.length === 0
  const noQuestionsQuiz = initQuestions.length === 0  // quiz não bloqueará se não há perguntas
  ...
  // ── Gravar conclusão automaticamente quando não há quiz ─────────────
  // Etapas só com materiais (sem perguntas) nunca passavam pelo fluxo de
  // quiz (finishQuiz), que é o único lugar que gravava a conclusão no
  // banco (via submitQuiz). Isso fazia o botão de avançar aparecer (o
  // cálculo no navegador ficava satisfeito), mas o servidor nunca ficava
  // sabendo que a etapa foi concluída — e a próxima continuava bloqueada.
  const [autoCompleteSent, setAutoCompleteSent] = useState(false)
  useEffect(() => {
    if (isAdmin || hasNoContent) return // etapa vazia já tem seu próprio fluxo (completeEmptyStep)
    if (!allMaterialsChecked || !noQuestionsQuiz) return
    if (autoCompleteSent || userProgress?.status === 'concluido') return
    setAutoCompleteSent(true)
    submitQuiz(stepId, {}, quizScore, true)
  }, [allMaterialsChecked, noQuestionsQuiz, isAdmin, hasNoContent, autoCompleteSent, userProgress, quizScore, stepId])
  ...
  // Estado de conclusão manual (etapa sem conteúdo)
  const [completing, setCompleting] = useState(false)
  const [completed,  setCompleted]  = useState(userProgress?.status === 'concluido')

  // Conclui etapa que não tem material nem quiz
  async function completeEmptyStep() {
    setCompleting(true)
    await submitQuiz(stepId, {}, 100, true)
    setCompleted(true)
    setCompleting(false)
  ...
  return (
    <div>
      {/* Header da etapa */}
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 24, padding: '26px 28px', marginBottom: 20, position: 'relative', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: teamGrad }} />
        <div style={{ paddingTop: 6, display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap', marginBottom: 8 }}>
              <h1 style={{ fontSize: 23, fontWeight: 800, color: 'var(--foreground)', margin: 0, letterSpacing: '-0.02em' }}>{s.title}</h1>
              <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 999, background: `${teamColor}15`, color: teamColor, border: `1px solid ${teamColor}30` }}>{s.team}</span>
              {s.day_number && <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 999, background: teamGrad, color: '#fff' }}>Dia {s.day_number}</span>}
              {(userProgress?.status === 'concluido') && <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 999, background: 'rgba(34,197,94,0.1)', color: '#16a34a', border: '1px solid rgba(34,197,94,0.2)' }}>✓ Concluído</span>}
            </div>
            {s.description && <p style={{ fontSize: 14, color: 'var(--muted-foreground)', margin: '0 0 14px', lineHeight: 1.6, maxWidth: 640 }}>{s.description}</p>}
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {s.estimated_minutes && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11.5, fontWeight: 600, color: 'var(--muted-foreground)', background: 'var(--secondary)', padding: '5px 12px', borderRadius: 999 }}>⏱ {s.estimated_minutes} min</span>}
              {localQuestions.length > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11.5, fontWeight: 600, color: 'var(--muted-foreground)', background: 'var(--secondary)', padding: '5px 12px', borderRadius: 999 }}>🎯 Nota mínima: {s.min_quiz_score}%</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      {isSequential && !isAdmin && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 18, padding: '14px 18px', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', boxShadow: 'var(--shadow-xs)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 999, background: allMaterialsChecked ? 'rgba(34,197,94,0.08)' : 'var(--secondary)' }}>
            {allMaterialsChecked ? <CheckCircle2 size={15} style={{ color: '#22c55e' }} /> : <Circle size={15} style={{ color: 'var(--muted-foreground)' }} />}
            <span style={{ fontSize: 12.5, fontWeight: 600, color: allMaterialsChecked ? '#16a34a' : 'var(--muted-foreground)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 999, background: (quizFinished && quizScore >= s.min_quiz_score) ? 'rgba(34,197,94,0.08)' : 'var(--secondary)' }}>
            ...
            <span style={{ fontSize: 12.5, fontWeight: 600, color: quizFinished && quizScore >= s.min_quiz_score ? '#16a34a' : 'var(--muted-foreground)' }}>
              {localQuestions.length > 0 ? (quizFinished ? `Quiz: ${quizScore}%` : 'Quiz pendente') : 'Sem quiz'}
            </span>
          </div>
          {canAdvance && (
            <button onClick={handleNextStep} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 36, padding: '0 16px', borderRadius: 999, background: 'linear-gradient(135deg,#22c55e,#16a34a)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 12.5, fontWeight: 700, fontFamily: 'inherit', boxShadow: '0 3px 10px rgba(34,197,94,0.3)', marginLeft: 'auto' }}>
      ...
      {/* ── Etapa sem conteúdo: botão de concluir ── */}
      {!isAdmin && hasNoContent && !(completed || userProgress?.status === 'concluido') && (
        <div style={{ background:'var(--card)', border:'1.5px solid rgba(99,102,241,.25)', borderRadius:22, padding:'26px', marginBottom:20, textAlign:'center', boxShadow:'0 2px 12px rgba(99,102,241,.08)' }}>
          <div style={{ fontSize:40, marginBottom:10 }}>📋</div>
          <p style={{ fontSize:14, fontWeight:700, color:'var(--foreground)', margin:'0 0 6px' }}>Esta etapa não possui materiais ou quiz.</p>
          <p style={{ fontSize:13, color:'var(--muted-foreground)', margin:'0 0 18px' }}>Clique abaixo para marcar como concluída e avançar.</p>
          <button onClick={completeEmptyStep} disabled={completing}
            style={{ height:48, padding:'0 34px', borderRadius:999, background:completing?'var(--secondary)':'linear-gradient(135deg,#4f46e5,#7c3aed)', color:completing?'var(--muted-foreground)':'#fff', fontSize:14, fontWeight:800, border:'none', cursor:completing?'not-allowed':'pointer', fontFamily:'inherit', boxShadow:completing?'none':'0 4px 16px rgba(79,70,229,.35)', display:'inline-flex', alignItems:'center', gap:8 }}>
            {completing ? 'Concluindo...' : <><CheckCircle2 size={16}/> Concluir Etapa <ArrowRight size={14}/></>}
          </button>
        </div>
      )}

      {/* Etapa sem conteúdo e já concluída */}
      {!isAdmin && hasNoContent && (completed || userProgress?.status === 'concluido') && (
        <div style={{ background:'rgba(34,197,94,.06)', border:'1px solid rgba(34,197,94,.25)', borderRadius:18, padding:'16px 20px', marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
          <span style={{ fontSize:13.5, fontWeight:700, color:'#16a34a', display:'flex', alignItems:'center', gap:6 }}>
            <CheckCircle2 size={16}/> Etapa concluída!
          </span>
          <button onClick={handleNextStep}
            style={{ height:38, padding:'0 20px', borderRadius:999, background:'linear-gradient(135deg,#22c55e,#16a34a)', color:'#fff', border:'none', cursor:'pointer', fontSize:13, fontWeight:700, fontFamily:'inherit', display:'flex', alignItems:'center', gap:6 }}>
            {nextStep ? 'Próxima etapa' : 'Voltar à trilha'} <ArrowRight size={13}/>
          </button>
        </div>
      )}

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 5, marginBottom: 20, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 18, padding: 5, boxShadow: 'var(--shadow-xs)' }}>
        {tabs.map(tab => {
          const Icon = tab.icon
          const active = activeTab === tab.id
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, height: 42, borderRadius: 14, border: 'none', background: active ? 'linear-gradient(135deg,#4f46e5,#7c3aed)' : 'transparent', color: active ? '#fff' : 'var(--muted-foreground)', fontSize: 13, fontWeight: active ? 700 : 500, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', boxShadow: active ? '0 4px 12px rgba(79,70,229,0.3)' : 'none' }}>
              <Icon size={14} />
              {tab.label}
              {tab.count > 0 && <span style={{ fontSize: 9.5, fontWeight: 800, padding: '1px 6px', borderRadius: 999, background: active ? 'rgba(255,255,255,0.25)' : 'var(--secondary)', color: active ? '#fff' : 'var(--muted-foreground)' }}>{tab.count}</span>}
            </button>
          )
        })}
      </div>
      ...
      {activeTab === 'materiais' && (
        <div>
          ...
          {materials.length === 0
            ? <div style={{ textAlign: 'center', padding: '36px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 20 }}><p style={{ fontSize: 13.5, color: 'var(--muted-foreground)' }}>{isAdmin ? 'Nenhum material ainda. Adicione o primeiro!' : 'Nenhum material disponível nesta etapa.'}</p></div>
      ...
      {activeTab === 'quiz' && (
        <div>
          {isAdmin ? (
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 26, padding: '44px 32px', textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 26, padding: '44px 32px', textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#8b5cf6,#a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', boxShadow: '0 8px 24px rgba(139,92,246,0.3)' }}><CheckSquare size={30} style={{ color: '#fff' }} /></div>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--foreground)', marginBottom: 8 }}>Quiz — {s.title}</h2>
              <p style={{ fontSize: 14, color: 'var(--muted-foreground)', marginBottom: 10 }}>{localQuestions.length} questões · Nota mínima: {s.min_quiz_score}%</p>
              {isSequential && !allMaterialsChecked && <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 16px', borderRadius: 999, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', marginBottom: 22, fontSize: 12, color: '#d97706', fontWeight: 600 }}><Lock size={12} /> Conclua todos os materiais antes de fazer o quiz</div>}
              <br />
              <button onClick={() => setQuizStarted(true)} disabled={isSequential && !allMaterialsChecked}
                style={{ height: 50, padding: '0 38px', borderRadius: 999, background: isSequential && !allMaterialsChecked ? 'var(--secondary)' : 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: isSequential && !allMaterialsChecked ? 'var(--muted-foreground)' : '#fff', fontSize: 15, fontWeight: 800, border: 'none', cursor: isSequential && !allMaterialsChecked ? 'not-allowed' : 'pointer', fontFamily: 'inherit', boxShadow: isSequential && !allMaterialsChecked ? 'none' : '0 6px 20px rgba(79,70,229,0.4)', marginTop: 8 }}>
                {isSequential && !allMaterialsChecked ? '🔒 Bloqueado' : 'Iniciar Quiz 🚀'}
              </button>
            </div>
          ) : (
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 26, padding: '26px', boxShadow: 'var(--shadow-sm)' }}>
                ...
                {currentQuestion?.onboarding_answers?.map((a: any) => {
                  ...
                  return (
                    <button key={a.id} onClick={() => !selectedAnswer && selectAnswer(currentQuestion.id, a.id)} disabled={!!selectedAnswer}
                      style={{ width: '100%', padding: '13px 16px', borderRadius: 16, border: `1.5px solid ${border}`, background: bg, color, fontSize: 13.5, fontWeight: isSel ? 700 : 500, cursor: selectedAnswer ? 'default' : 'pointer', fontFamily: 'inherit', textAlign: 'left', transition: 'all 0.18s', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {showExpl && (
                <button onClick={nextQuestion} disabled={submitting} style={{ width: '100%', height: 48, borderRadius: 999, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 14px rgba(79,70,229,0.3)' }}>
