...
import {
  ...
  CheckCircle2, Circle, Lock, ArrowRight, Trophy
} from 'lucide-react'
  ...
  return (
    <button onClick={onClick}
      style={{
        ...
        background: checked ? 'rgba(34,197,94,0.1)' : 'var(--secondary)',
        color: checked ? '#16a34a' : 'var(--muted-foreground)',
        cursor: 'pointer', fontSize: 12, fontWeight: 700,
        fontFamily: 'inherit', transition: 'all 0.18s',
        boxShadow: checked ? '0 2px 8px rgba(34,197,94,0.2)' : 'var(--shadow-xs)',
      }}
      onMouseEnter={e => { if (!checked) { e.currentTarget.style.background = 'rgba(34,197,94,0.06)'; e.currentTarget.style.borderColor = '#86efac'; e.currentTarget.style.color = '#16a34a' } }}
      onMouseLeave={e => { if (!checked) { e.currentTarget.style.background = 'var(--secondary)'; e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--muted-foreground)' } }}
...
// ── MaterialCardInline ────────────────────────────────────────
function MaterialCardInline({ mat, isAdmin, onDeleted, onUpdated, onCheck, allSteps = [] }: {
  mat: any; isAdmin: boolean; allSteps?: { id: string; title: string; day_number: number | null }[]
  onDeleted: (id: string) => void
  onUpdated: (mat: any) => void
  ...
  // Select concatenado: etapa atual do material
  ...
  async function handleCheck(e: React.MouseEvent) {
    ...
    await fetch('/api/onboarding/check', {
      ...
      body: JSON.stringify({ type: 'material', id: mat.id, completed: next }),
    })
  }
  ...
  async function handleSaveEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: any = {
      ...
      url:         fd.get('url'),
      type:        fd.get('type'),
      step_id:     selStep || mat.step_id,  // muda a etapa no banco
      // day_number não existe em onboarding_materials
    }
    ...
    if (res.ok) {
      // Atualiza UI com novo stepInfo baseado na etapa escolhida
      onUpdated({
        ...mat,
        ...payload,
        step_id: selStep || mat.step_id,
      })
      setEditing(false)
    }
    setSaving(false)
      ...
      <form onSubmit={handleSaveEdit} style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
          <div><label style={{ fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Tipo</label>
            <StyledSelectInline name="type" defaultValue={mat.type}>
              <option value="video">🎬 Vídeo</option><option value="documento">📄 Documento</option>
              <option value="pdf">📑 PDF</option><option value="apresentacao">📊 Apresentação</option>
              <option value="site">🌐 Site</option><option value="outro">📎 Outro</option>
            </StyledSelectInline>
          </div>
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <select
                value={selStep}
                onChange={e => setSelStep(e.target.value)}
                style={{ ...inp, paddingRight: 28, appearance: 'none', WebkitAppearance: 'none', cursor: 'pointer', fontSize: 12 }}>
                {[...allSteps]
                  .sort((a, b) => {
            ...
            {(() => { const s = allSteps.find(x => x.id === selStep); return s?.day_number ? <p style={{ fontSize: 10, color: '#6366f1', marginTop: 3, fontWeight: 600 }}>📅 Dia {s.day_number}</p> : null })()}
          </div>
        </div>
  ...
  return (
    <>
      ...
      {/* Card — altura fixa via flex para padronizar botões */}
      <div style={{
        ...
        {thumb && mat.type === 'video' && (
          <div onClick={() => setPlaying(true)}
            style={{ position: 'relative', height: 130, cursor: 'pointer', background: '#000', overflow: 'hidden', flexShrink: 0 }}>
        ...
        {/* Conteúdo — flex grow para empurrar footer para baixo */}
        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          {/* Descrição com altura mínima para alinhar o footer */}
          <p style={{ fontSize: 11, color: 'var(--muted-foreground)', margin: '0', lineHeight: 1.5, flex: 1, minHeight: 0 }}>
            {mat.description ?? ''}
          </p>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
            <button
              onClick={() => embedUrl ? setPlaying(true) : window.open(mat.url, '_blank')}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, height: 30, padding: '0 12px', borderRadius: 8, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: 'inherit', boxShadow: '0 2px 8px rgba(79,70,229,0.3)' }}>
            ...
            {/* Check — visível e claro, empurrado pra direita */}
            {!isAdmin && (
...
function FaqCard({ faq, isAdmin, expanded, onToggle, onDeleted, onUpdated }: any) {
  ...
  async function handleDelete() {
    setSaving(true)
    await fetch('/api/admin/faqs', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: faq.id }) })
    onDeleted(faq.id)
    setSaving(false)
  }
  ...
  if (editing) return (
    <div style={{ background: 'var(--card)', border: '1.5px solid rgba(99,102,241,0.3)', borderRadius: 13, padding: '14px 16px' }}>
      <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        <div><label style={{ fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Pergunta *</label>
          <input name="question" required defaultValue={faq.question} style={inp} /></div>
        <div><label style={{ fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Resposta *</label>
          <textarea name="answer" required defaultValue={faq.answer} rows={5} style={{ ...inp, height: 'auto', padding: '8px 12px', resize: 'vertical', lineHeight: 1.6 }} /></div>
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 13, overflow: 'hidden', boxShadow: 'var(--shadow-xs)', transition: 'border-color 0.15s' }}
      onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(99,102,241,0.2)'}
      onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={onToggle}
          style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 16px', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', gap: 10 }}>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', gap: 4, paddingRight: 10, flexShrink: 0 }}>
            <button onClick={() => setEditing(true)} style={{ width: 26, height: 26, borderRadius: 6, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(99,102,241,0.1)'; e.currentTarget.style.color = '#6366f1'; e.currentTarget.style.borderColor = 'rgba(99,102,241,0.3)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)'; e.currentTarget.style.borderColor = 'var(--border)' }}>
              <Pencil size={11} />
            </button>
            <button onClick={() => setDeleting(true)} style={{ width: 26, height: 26, borderRadius: 6, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)', transition: 'all 0.12s' }}
              onMouseEnter={e => { e.currentTarget.style.background = 'rgba(239,68,68,0.08)'; e.currentTarget.style.color = '#ef4444'; e.currentTarget.style.borderColor = 'rgba(239,68,68,0.3)' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--muted-foreground)'; e.currentTarget.style.borderColor = 'var(--border)' }}>
              <Trash2 size={11} />
            </button>
      ...
      {expanded && (
        <div style={{ padding: '10px 16px 14px', fontSize: 13, color: 'var(--muted-foreground)', lineHeight: 1.65, borderTop: '1px solid var(--border)' }}>
          {faq.answer}
        </div>
      )}
    </div>
...
export function StepDetail({
  step, materials: initMaterials, faqs: initFaqs, questions,
  ...
  const [materials,   setMaterials]   = useState(
    initMaterials.map((m: any) => ({ ...m, checked: initChecked.includes(m.id) }))
  )
  const [faqs, setFaqs] = useState(initFaqs)

  // Track quais materiais estão checked
  ...
  // Quiz — inicializa com progresso já salvo
  ...
  // Verifica se pode avançar no modo sequencial
  ...
  function selectAnswer(qId: string, aId: string) {
    setSelectedAns(prev => ({ ...prev, [qId]: aId }))
    setShowExpl(true)
  }
    ...
    <div>
        ...
        <div style={{ paddingTop: 8, display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              {questions.length > 0 && <span style={{ fontSize: 11, color: 'var(--muted-foreground)' }}>🎯 Nota mínima: {s.min_quiz_score}%</span>}
            </div>
          </div>
        </div>
      </div>

      {/* Barra de progresso da etapa (modo sequencial) */}
          ...
          {canAdvance && (
            <>
              ...
              <button onClick={handleNextStep}
                style={{ display: 'flex', alignItems: 'center', gap: 6, height: 32, padding: '0 14px', borderRadius: 9, background: 'linear-gradient(135deg,#22c55e,#16a34a)', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'inherit', boxShadow: '0 3px 10px rgba(34,197,94,0.3)', marginLeft: 'auto' }}>
      ...
      {activeTab === 'materiais' && (
        <div>
          ...
          {materials.length === 0
            ? <div style={{ textAlign: 'center', padding: '32px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, boxShadow: 'var(--shadow-xs)' }}>
                <p style={{ fontSize: 13, color: 'var(--muted-foreground)' }}>{isAdmin ? 'Nenhum material ainda. Adicione o primeiro!' : 'Nenhum material disponível nesta etapa.'}</p>
              </div>
            : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(280px,100%),1fr))', gap: 14 }}>
                {materials.map((m: any) => (
                  <MaterialCardInline key={m.id} mat={m} isAdmin={isAdmin} allSteps={allSteps}
                    onCheck={handleMaterialCheck}
                    onDeleted={id => setMaterials((prev: any) => prev.filter((x: any) => x.id !== id))}
                    onUpdated={updated => setMaterials((prev: any) => prev.map((x: any) => x.id === updated.id ? { ...updated, checked: x.checked } : x))}
                  />
                ))}
              </div>
          }
      ...
      {activeTab === 'faqs' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {faqs.map((f: any) => (
            <FaqCard key={f.id} faq={f} isAdmin={isAdmin}
      ...
      {activeTab === 'quiz' && (
        <div>
          {isAdmin ? (
            ...
            <div style={{ textAlign: 'center', padding: '32px', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 14, boxShadow: 'var(--shadow-xs)' }}>
              <p style={{ fontSize: 13, color: 'var(--muted-foreground)' }}>Quiz ainda não disponível para esta etapa.</p>
            </div>
          ) : quizFinished ? (
            // Resultado já salvo ou recém finalizado
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 18, padding: '40px 32px', textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
              ...
              <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--foreground)', marginBottom: 6 }}>
                {quizScore >= s.min_quiz_score ? 'Aprovado! Você arrasou! 🎉' : 'Continue tentando!'}
              </p>
              <p style={{ fontSize: 13, color: 'var(--muted-foreground)', marginBottom: 28 }}>Nota mínima: {s.min_quiz_score}%</p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                <button onClick={() => { setQuizFinished(false); setQuizStarted(false); setCurrentQ(0); setSelectedAns({}); setShowExpl(false) }}
                  style={{ height: 40, padding: '0 22px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--foreground)', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', boxShadow: 'var(--shadow-xs)' }}>
                  Tentar novamente
                </button>
                {canAdvance && (
                  <button onClick={handleNextStep}
                    style={{ height: 40, padding: '0 22px', borderRadius: 10, background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', fontSize: 14, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 7, boxShadow: '0 4px 14px rgba(79,70,229,0.3)' }}>
                    {nextStep ? 'Próxima etapa' : 'Finalizar trilha'} <ArrowRight size={14} />
                  </button>
                )}
                {!canAdvance && isSequential && !allMaterialsChecked && (
                  <div style={{ padding: '8px 14px', borderRadius: 9, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', fontSize: 12, color: '#d97706', fontWeight: 600 }}>
                    ⚠ Conclua todos os materiais para avançar
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 18, padding: '40px 32px', textAlign: 'center', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'linear-gradient(135deg,#8b5cf6,#a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: '0 8px 24px rgba(139,92,246,0.3)' }}>
                <CheckSquare size={28} style={{ color: '#fff' }} />
              </div>
              <h2 style={{ fontSize: 19, fontWeight: 800, color: 'var(--foreground)', marginBottom: 8 }}>Quiz — {s.title}</h2>
              <p style={{ fontSize: 14, color: 'var(--muted-foreground)', marginBottom: 8 }}>{questions.length} questões · Nota mínima: {s.min_quiz_score}%</p>
              {isSequential && !allMaterialsChecked && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 14px', borderRadius: 9, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', marginBottom: 20, fontSize: 12, color: '#d97706', fontWeight: 600 }}>
                  <Lock size={12} /> Conclua todos os materiais antes de fazer o quiz
                </div>
              )}
              <br />
                ...
                {currentQuestion?.onboarding_answers?.map((a: any) => {
                  ...
                  return (
                    <button key={a.id} onClick={() => !selectedAnswer && selectAnswer(currentQuestion.id, a.id)} disabled={!!selectedAnswer}
                      style={{ width: '100%', padding: '11px 15px', borderRadius: 11, border: `1.5px solid ${border}`, background: bg, color, fontSize: 13, fontWeight: isSel ? 700 : 500, cursor: selectedAnswer ? 'default' : 'pointer', fontFamily: 'inherit', textAlign: 'left', transition: 'all 0.18s', display: 'flex', alignItems: 'center', gap: 10, boxShadow: 'var(--shadow-xs)' }}>
