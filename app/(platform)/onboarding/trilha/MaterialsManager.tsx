...
export function MaterialsManager({ stepId, allSteps=[], onCreated }: {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = { step_id:selStep, title:fd.get('title'), description:(fd.get('description') as string)||null, url, type:matType }
    let res: Response, data: any
    try {
      res  = await fetch('/api/admin/materials', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
      data = await res.json()
    } catch(e: any) {
      setError('Erro de conexão: ' + (e.message ?? 'tente novamente'))
            ...
            <form onSubmit={handleSubmit} style={{ padding:'20px 22px', display:'flex', flexDirection:'column', gap:13 }}>
              ...
              <div>
                ...
                <SS value={selStep} onChange={setSelStep}>
                  {sorted.map(s => (<option key={s.id} value={s.id}>{s.day_number!==null ? `Dia ${s.day_number} - ${s.title}` : s.title}</option>))}
                </SS>
              ...
              <div>
                ...
                <input name="url" required type="url" value={url} onChange={e=>handleUrl(e.target.value)} placeholder="YouTube, Vimeo, Loom, Drive, PDF..." style={inp} onFocus={foc} onBlur={blr}/>
              ...
              <div>
                ...
                <SS value={matType} onChange={setMatType}>
                  ...
                  <option value="site">🌐 Site / Link</option><option value="outro">📎 Outro</option>
                </SS>
              </div>
...
export function QuizManager({ stepId, onCreated }: { stepId:string; onCreated?:(q:any)=>void }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const fd = new FormData(e.currentTarget)
    const questionText = (fd.get('question') as string)?.trim()
    if (!questionText) { setError('Digite a pergunta antes de salvar.'); setLoading(false); return }

    const filled = answers.filter(a => a.text.trim())
    if (filled.length < 2) { setError('Preencha pelo menos 2 alternativas.'); setLoading(false); return }

    const hasCorrect = answers.some(a => a.is_correct)
    if (!hasCorrect) { setError('⚠ Selecione qual alternativa é a correta clicando no círculo verde.'); setLoading(false); return }

    const res  = await fetch('/api/admin/questions', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ step_id:stepId, question:questionText, explanation:(fd.get('explanation') as string)||null, answers }) })
    const data = await res.json()
    if (!res.ok) { setError(data.error ?? 'Erro'); setLoading(false); return }
  ...
  return (
    <>
      ...
      {open && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', backdropFilter:'blur(6px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }} onClick={e=>e.target===e.currentTarget&&setOpen(false)}>
          <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:22, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 28px 64px rgba(0,0,0,0.25)' }}>
            <div style={{ background:'linear-gradient(135deg,#2e1065,#3730a3,#4f46e5)', borderRadius:'22px 22px 0 0', padding:'18px 22px' }}>
              <h2 style={{ fontSize:16, fontWeight:900, color:'#fff', margin:0 }}>Adicionar questão ao quiz</h2>
              <p style={{ fontSize:12, color:'rgba(255,255,255,0.65)', margin:'4px 0 0' }}>Selecione o círculo da alternativa correta ✓</p>
            </div>
            <form onSubmit={handleSubmit} style={{ padding:'20px 22px', display:'flex', flexDirection:'column', gap:13 }}>
              {error && <div style={{ padding:'10px 14px', borderRadius:9, background:'rgba(239,68,68,0.08)', border:'1.5px solid rgba(239,68,68,0.3)' }}><p style={{ fontSize:12, color:'#ef4444', margin:0, fontWeight:600 }}>{error}</p></div>}
              ...
              <div>
                <label style={lbl}>
                  Alternativas
                  <span style={{ color: answers.some(a=>a.is_correct) ? '#22c55e' : '#ef4444', fontWeight:800, marginLeft:6 }}>
                    {answers.some(a=>a.is_correct) ? '✓ Correta selecionada' : '* Selecione a correta'}
                  </span>
                </label>
