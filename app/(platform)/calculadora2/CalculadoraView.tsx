...
// preenchidos, procura entre as "linhas irmãs" do mesmo produto por uma
// que tenha). Extraída como função separada porque agora precisa rodar
// tanto pro produto principal quanto pro produto de upsell — antes só
// rodava pro principal, e por isso o upsell nunca aparecia nos entregáveis.
function findEntregaveis(row: PriceRow | null, rows: PriceRow[]): string {
  if (!row) return ''
  if (row.entregaveis) return row.entregaveis
  const irma = rows.find(r =>
    r.vertical === row.vertical && r.produto === row.produto &&
    r.tempoAcesso === row.tempoAcesso && r.tipoAluno === row.tipoAluno &&
    r.canalVenda === row.canalVenda && r.entregaveis
  )
...
export function CalculadoraView({ isAdmin = false, userTeam = null }: Props) {
  ...
  // Entregáveis do produto principal + do upsell (quando o combo está
  // ativo), combinados. Antes só considerava o produto principal — por
  // isso, ao adicionar um segundo produto, os entregáveis dele nunca
  // apareciam na mensagem/copy final.
  const entregaveisFinal = useMemo(() => {
    const principal = findEntregaveis(selectedRow, rows)
    if (!upsellOn || !upsellRow) return principal
    const doUpsell = findEntregaveis(upsellRow, rows)
    if (!principal) return doUpsell
    if (!doUpsell) return principal
    return `${principal} + ${doUpsell}`
  }, [selectedRow, upsellOn, upsellRow, rows])
